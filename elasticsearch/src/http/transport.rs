    ...
    TStrategy: Strategy + Clone,
{
    fn next(&self) -> &Connection {
        self.strategy
            .try_next(&self.connections)
            .unwrap()
    }
}

impl<TStrategy> StaticNodeListConnectionPool<TStrategy> {
    pub fn set(&mut self, connections: Vec<Connection>) -> Result<(), Error> {
        if connections.is_empty() {
            return Err(crate::error::lib("Node count should be greater than 1"));
        }
...
impl StaticNodeListConnectionPool<RoundRobin> {
    ...
    {
        let connections: Vec<_> = urls.into_iter().map(|url| Connection::new(url)).collect();
...
pub mod tests {
    #[cfg(any(feature = "native-tls", feature = "rustls-tls"))]
    use crate::auth::ClientCertificate;
    use crate::http::transport::{CloudId, Connection, SingleNodeConnectionPool, TransportBuilder, StaticNodeListConnectionPool, RoundRobin, ConnectionPool};
    use url::Url;
    ...
    fn expected_addresses() -> Vec<Url> {
        vec!["http://a:9200/", "http://b:9200/", "http://c:9200/"]
            .iter()
            .map(|addr| Url::parse(addr).unwrap()).collect()
    }
